...
class TelegramWebhookController extends Controller
{
    ...
    {
        // Temporary — remove after confirming messages are received
        ...
        // ----------------------------------------------------------------
        // ABA payment — check FIRST before any bot filtering
        // Handles: "$100 paid by", "៛100 paid by", "...100 paid by"
        // ----------------------------------------------------------------
        if (
    ...
    private function storeAbaPayment(Request $request, string $text)
    {
        try {
            ...
            // ----------------------------------------------------------------
            // Clean text — strip any prefixes ABA bot might add
            // ----------------------------------------------------------------
            $cleanText = $text;
            $cleanText = preg_replace('/^@\S+\s+/u', '', $cleanText);      // @BotName prefix
            $cleanText = preg_replace('/^\[.*?\]:\s*/us', '', $cleanText);  // [date header]:
            $cleanText = preg_replace('/^\.{3}/u', '', $cleanText);         // ... truncation
            $cleanText = trim($cleanText);

            // ----------------------------------------------------------------
            // Detect currency BEFORE regex
            // Check raw cleanText for $ or ៛ at the very start
            // ----------------------------------------------------------------
            $firstChar = mb_substr($cleanText, 0, 1, 'UTF-8');
            if ($firstChar === '៛') {
                $currency = 'KHR';
            } else {
                $currency = 'USD'; // $ or anything else defaults to USD
            }

            // ----------------------------------------------------------------
            // Regex
            // Strip currency symbol from start so amount captures cleanly
            // Handles: $100, ៛100, 100 (after ... strip)
            // Also handles both:
            //   via ABA KHQR (Bakong) at ...
            //   via ABA KHQR (BKRTKHPPXXX) at ...
            // ----------------------------------------------------------------
            $regexText = preg_replace('/^[៛\$＄]/u', '', $cleanText);
            $regexText = trim($regexText);

            preg_match(
                '/(?P<amount>[\d,]+(?:\.\d+)?)\s+paid\s+by\s+(?P<payer_name>.+?)\s+\((?P<payer_account>[^)]+)\)\s+on\s+(?P<date>[A-Za-z]+\s+\d{1,2},\s*\d{1,2}:\d{2}\s*(?:AM|PM))\s+via\s+(?P<method>[^(]+?)\s+\((?P<bank_code>[^)]+)\)\s+at\s+(?P<merchant>.+?)\.\s+Trx\.\s*ID:\s*(?P<trx_id>\d+),\s*APV:\s*(?P<apv>\d+)/ui',
                $regexText,
                $match
            );
            ...
            // ----------------------------------------------------------------
            // Date — "Jun 12, 05:17 PM"
            // ----------------------------------------------------------------
            try {
                $paymentDate = Carbon::createFromFormat('M j, h:i A', trim($match['date']))
                    ->year(now()->year);
            } catch (\Throwable $e) {
                try {
            ...
            $payment = TelegramPayment::updateOrCreate(
                ...
                [
                    ...
                    'payment_method'      => trim($match['method']),  // "ABA KHQR"
                    'bank_code'           => trim($match['bank_code']),// "Bakong" or "BKRTKHPPXXX"
                    'trx_id'              => $trxId,
                    'apv'                 => trim($match['apv']),
    ...
    public function setWebhook()
    {
        ...
        return $this->telegramApi('setWebhook', ['url' => $url]);
    }
